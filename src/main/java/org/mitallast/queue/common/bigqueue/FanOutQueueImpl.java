...
import org.mitallast.queue.common.bigqueue.page.IMappedPage;
...
import org.mitallast.queue.common.bigqueue.utils.FolderNameValidator;
...
public class FanOutQueueImpl implements IFanOutQueue {
    ...
    private final ConcurrentMap<String, QueueFront> queueFrontMap = new ConcurrentHashMap<String, QueueFront>();
     ...
     * @param pageSize  the back data file size per page in bytes, see minimum allowed {@link private .bigqueue.BigArrayImpl#MINIMUM_DATA_PAGE_SIZE}
     ...
     * use default back data page size, see {@link private .bigqueue.BigArrayImpl#DEFAULT_DATA_PAGE_SIZE}
     *
     * @param queueDir  the directory to store queue data
     * @param queueName the name of the queue, will be appended as last part of the queue directory
     * @throws IOException exception throws if there is any IO error during queue initialization
