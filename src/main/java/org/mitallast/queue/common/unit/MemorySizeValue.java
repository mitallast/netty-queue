        ...
        if (sValue.endsWith("%")) {
            ...
            try {
                ...
                if (percent < 0 || percent > 100) {
                    throw new IllegalArgumentException("Percentage should be in [0-100], got " + percentAsString);
                }
                return new ByteSizeValue((long) ((percent / 100) * Runtime.getRuntime().maxMemory()), ByteSizeUnit.BYTES);
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException("Failed to parse [" + percentAsString + "] as a double", e);
            }
        } else {
