...
class DefaultCrdtService @Inject
constructor(
        ...
        if (bucket == null) {
            ...
            try {
                ...
                if (replica == null) {
                    ...
                    var localIndex = bucket.state().get(message.replica())
    ...
    private fun handle(message: ClosedLogSynced) {
        ...
        try {
            ...
            val replica = routingTable.bucket(message.bucket().toLong())
                ...
                for (resource in routingBucket.resources().values()) {
                    if (bucket.registry().crdtOpt(resource.id()).isEmpty) {
                        ...
                        when (resource.type()) {
                            ...
                            else -> logger.warn("unexpected type: {}", resource.type())
                        }
                    }
                }
            }
