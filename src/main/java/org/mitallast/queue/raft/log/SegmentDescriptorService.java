...
public class SegmentDescriptorService extends AbstractComponent {
    ...
    public ImmutableList<SegmentDescriptor> loadDescriptors() throws IOException {
        ...
        for (SegmentFile segmentFile : fileService.listDescriptorFiles()) {
            SegmentDescriptor descriptor = readDescriptor(segmentFile);
    ...
    private SegmentDescriptor readDescriptor(SegmentFile segmentFile) throws IOException {
        logger.info("read segment {}:{}", segmentFile.id(), segmentFile.version());
        try (StreamInput input = streamService.input(segmentFile.file())) {
