...
public class RestTransactionDeleteAction extends BaseRestHandler {
    ...
    @Inject
    public RestTransactionDeleteAction(Settings settings, RestController controller, TransportService transportService) {
        super(settings);
    ...
    public void handleRequest(final RestRequest request, final RestSession session) {
        ...
        transportService.client().send(builder.build()).whenComplete((response, error) -> {
            if (error == null) {
